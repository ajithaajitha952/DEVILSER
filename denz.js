...
try {
		...
		const dfrply = fs.readFileSync('./Ammukutty.jpg')
		cmhit.push(command)
        mess = {
			wait: '𝚆𝙰𝙸𝚃💖',
			success: '𝚂𝚄𝙲𝙲𝚄𝚂𝚂✔',
...
denz.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./Ammukutty.jpg')})
break
case 'nuliskanan':
if (!c) return reply('Wheres the text bro?')
reply(mess.wait)
...
//My Api
       case 'maker2d2': 
					...
					denz.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./Ammukutty.jpg')})
					break
		case 'maker2d3': 
                    if (args.length < 1) return reply(`*Example :*\n${prefix}${command}ꪶ𝑨𝑴𝑬𝑬𝑵⇥𝑺𝑬𝑹-𝑩𝑶𝑻ꫂ⁩`)
					makell = args.join(" ")
...
case 'setthumb':
			...
			if (!arg) return reply
				...
				fs.unlinkSync(`./Ammukutty.jpg`)
                await sleep(1000)
				fs.writeFileSync(`./Ammukutty.jpg`, delb)
				denz.sendMessage(_.jid, buff, sticker, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 9999999999, status: 200, thumbnail: fs.readFileSync('./Ammukutty.jpg'), surface: 200, message: `${bc}`, orderTitle: `${bc}`, sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 508, isForwarded: true}})
				  break
case 'addcmd': 
case 'setcmd':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
Giliran = @${tty.player1.split('@')[0]}`
 denz.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer DEVILSER",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./Ammukutty.jpg'),sourceUrl:"https://wa.me/917994909899?text=Hi Bro"}}})
	}
    }
    }
    }
